    ...
    <div class="max-w-2xl w-full bg-white rounded-lg shadow-lg p-8">
        ...
        <div class="mb-6 p-4 bg-blue-50 rounded-lg">
            ...
            <div class="space-y-2 text-sm">
                <div class="flex justify-between">
                    ...
                    <span class="font-medium">{{ $signature->signable->letter_number ?? 'Draft' }}</span>
                </div>
                <div class="flex justify-between">
                    <span class="text-gray-600">Tipe:</span>
                    <span class="font-medium">{{ ucfirst($signature->signable->letter_type) }}</span>
                </div>
                <div class="flex justify-between">
                    <span class="text-gray-600">Judul:</span>
                    <span class="font-medium">{{ $signature->signable->subject }}</span>
                </div>
            </div>
        </div>
        @endif
        ...
        <div class="mb-6 p-4 bg-gray-50 rounded-lg">
            ...
            <div class="space-y-2 text-sm">
                <div class="flex justify-between">
                    ...
                    <span class="font-medium">{{ $signature->signer->name }}</span>
                </div>
                <div class="flex justify-between">
                    <span class="text-gray-600">Email:</span>
                    <span class="font-medium">{{ $signature->signer->email }}</span>
                </div>
                <div class="flex justify-between">
                    <span class="text-gray-600">Waktu:</span>
                    <span class="font-medium">{{ $signature->created_at->format('d M Y H:i:s') }}</span>
                </div>
                <div class="flex justify-between">
                    <span class="text-gray-600">IP Address:</span>
                    <span class="font-medium font-mono text-xs">{{ $signature->ip_address }}</span>
                </div>
            </div>
        </div>
        ...
        <div class="mb-6 p-4 bg-yellow-50 rounded-lg">
            ...
            <div class="space-y-3 text-sm">
                <div>
                    ...
                    <code class="block p-2 bg-white rounded border text-[10px] break-all font-mono leading-relaxed">{{ $signature->signature_hash }}</code>
                ...
                <div>
                    ...
                    <code class="block p-2 bg-white rounded border text-xs break-all font-mono">{{ $signature->verification_token }}</code>
                </div>
            </div>
        </div>

        <!-- Verification History -->
        @if($signature->verifications && $signature->verifications->count() > 0)
            ...
            <div class="space-y-3">
                ...
                <div class="text-sm bg-white p-3 rounded border">
                    <div class="flex justify-between mb-2">
                        <span class="font-medium text-gray-900">{{ $verification->verifier->name }}</span>
                        <span class="text-gray-600">{{ $verification->created_at->format('d M Y H:i') }}</span>
                    </div>
                    @if($verification->verification_note)
                    <p class="text-gray-600 text-xs">{{ $verification->verification_note }}</p>
                    @endif
                </div>
                @endforeach
            </div>
        ...
        <div class="mb-6">
            ...
            <div class="border rounded-lg p-4 bg-white">
                <img src="{{ $signature->signature_image }}" alt="Signature" class="max-w-full h-auto mx-auto" style="max-height: 200px;">
            </div>
        </div>
        @endif
        ...
        <div class="mt-8 pt-6 border-t text-center">
            <p class="text-xs text-gray-500">
                Dokumen ini diverifikasi melalui sistem HR AdministratorApp.<br>
