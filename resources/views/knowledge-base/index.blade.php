...
<div class="page-heading">
        ...
        <div class="row">
            <div class="col-12 col-md-6 order-md-1 order-last">
                <div class="d-flex align-items-center">
                    ...
                    @if(Auth::user()->hasAccess('knowledge_base'))
                        <a href="{{ route('knowledge-base.create') }}" class="btn btn-primary btn-sm ms-3">
    ...
    {{-- ================================================================== --}}
    {{-- SECTION 1: PANDUAN PENGGUNAAN APLIKASI (USER GUIDE) --}}
    {{-- ================================================================== --}}
    <section id="section-user-guide" class="kb-section">
        ...
        {{-- Dynamic User Guides --}}
        @if(isset($categories['user-guide']))
            @foreach($categories['user-guide'] as $article)
                <div class="card mb-3 kb-item" data-keywords="{{ $article->keywords }}">
                    <div class="card-header bg-white d-flex align-items-center" style="cursor:pointer">
                        <div class="flex-grow-1" data-bs-toggle="collapse" data-bs-target="#article-{{ $article->id }}">
                            <h5 class="mb-0">
                                <i class="bi bi-file-earmark-text text-primary me-2"></i> {{ $article->title }}
                                <i class="bi bi-chevron-down float-end"></i>
                            </h5>
                        </div>
                        @if(Auth::user()->hasAccess('knowledge_base'))
                            <div class="ms-3">
                                <a href="{{ route('knowledge-base.edit', $article->id) }}" class="btn btn-sm btn-outline-warning me-1"><i class="bi bi-pencil"></i></a>
                                <form action="{{ route('knowledge-base.destroy', $article->id) }}" method="POST" class="d-inline" onsubmit="return confirm('Hapus artikel ini?')">
                                    @csrf @method('DELETE')
    ...
    {{-- ================================================================== --}}
    {{-- SECTION 2: KEBIJAKAN & PERATURAN PERUSAHAAN --}}
    {{-- ================================================================== --}}
    <section id="section-company-policy" class="kb-section">
        ...
        @if(isset($categories['company-policy']))
            @foreach($categories['company-policy'] as $article)
                <div class="card mb-3 kb-item" data-keywords="{{ $article->keywords }}">
                    <div class="card-header bg-white d-flex align-items-center" style="cursor:pointer">
                        <div class="flex-grow-1" data-bs-toggle="collapse" data-bs-target="#article-{{ $article->id }}">
                            <h5 class="mb-0">
                                <i class="bi bi-shield-check text-success me-2"></i> {{ $article->title }}
                                <i class="bi bi-chevron-down float-end"></i>
                            </h5>
                        </div>
                        @if(Auth::user()->hasAccess('knowledge_base'))
    ...
    {{-- ================================================================== --}}
    {{-- SECTION 3: PANDUAN ADMIN / HR Administrator --}}
    {{-- ================================================================== --}}
    <section id="section-admin-guide" class="kb-section">
        ...
        {{-- Dynamic Admin Guides --}}
        @if(isset($categories['admin-guide']))
            @foreach($categories['admin-guide'] as $article)
                <div class="card mb-3 kb-item" data-keywords="{{ $article->keywords }}">
                    <div class="card-header bg-white d-flex align-items-center" style="cursor:pointer">
                        <div class="flex-grow-1" data-bs-toggle="collapse" data-bs-target="#article-{{ $article->id }}">
                            <h5 class="mb-0">
                                <i class="bi bi-gear text-warning me-2"></i> {{ $article->title }}
                                <i class="bi bi-chevron-down float-end"></i>
                            </h5>
                        </div>
                        @if(Auth::user()->hasAccess('knowledge_base'))
    ...
    <section id="section-faq" class="kb-section">
        ...
        <div class="accordion" id="faqAccordion">
            {{-- Dynamic FAQs --}}
            @if(isset($categories['faq']))
                @foreach($categories['faq'] as $article)
                    <div class="card mb-2 kb-item" data-keywords="{{ $article->keywords }}">
                        <div class="card-header bg-white d-flex align-items-center" style="cursor:pointer">
                            <div class="flex-grow-1" data-bs-toggle="collapse" data-bs-target="#article-{{ $article->id }}">
                                <h6 class="mb-0">
                                    <i class="bi bi-patch-question text-info me-2"></i> {{ $article->title }}
                                    <i class="bi bi-chevron-down float-end"></i>
                                </h6>
                            </div>
                            @if(Auth::user()->hasAccess('knowledge_base'))
            ...
            <div class="card mb-2 kb-item" data-keywords="{{ $faq['kw'] }}">
                <div class="card-header bg-white" style="cursor:pointer" data-bs-toggle="collapse" data-bs-target="#faq-{{ $i }}">
                    <h6 class="mb-0"><i class="bi bi-patch-question text-primary me-2"></i> {{ $faq['q'] }}</h6>
                </div>
