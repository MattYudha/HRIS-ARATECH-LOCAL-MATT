            ...
            <div class="card-body">
                <form action="{{ route('knowledge-base.update', $article->id) }}" method="POST">
                        ...
                        <div class="col-md-4">
                            <div class="form-group mb-3">
                                ...
                                <select name="category" id="category" class="form-select @error('category') is-invalid @enderror" required>
                                    <option value="user-guide" {{ $article->category == 'user-guide' ? 'selected' : '' }}>User Guide (Panduan Aplikasi)</option>
                                    <option value="company-policy" {{ $article->category == 'company-policy' ? 'selected' : '' }}>Company Policy (Kebijakan)</option>
                                    <option value="admin-guide" {{ $article->category == 'admin-guide' ? 'selected' : '' }}>Admin/HR Administrator Guide</option>
                    ...
                    <div class="form-group mb-3">
                        ...
                        <input type="text" name="keywords" id="keywords" class="form-control" value="{{ old('keywords', $article->keywords) }}" placeholder="contoh: kpi target penilaian">
                    </div>

                    <div class="form-group mb-3">
                        <label for="content" class="form-label">Konten Artikel (HTML diperbolehkan)</label>
                        <textarea name="content" id="content" class="form-control @error('content') is-invalid @enderror" rows="15" required>{{ old('content', $article->content) }}</textarea>
