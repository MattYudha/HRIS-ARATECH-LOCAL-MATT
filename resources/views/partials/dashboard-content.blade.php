        ...
        <div class="card shadow-sm border-0" style="background: linear-gradient(135deg, #667eea 0%, #764ba2 100%);">
            <div class="card-body px-4 py-4 text-white">
                <div class="d-flex align-items-center gap-3">
                    <img src="https://ui-avatars.com/api/?name={{ urlencode(auth()->user()->name ?? 'User') }}&background=ffffff&color=667eea&size=56"
                         class="rounded-circle" width="56" height="56">
                    <div>
                        <h4 class="mb-1 fw-bold">Selamat datang, {{ auth()->user()->name ?? 'User' }}!</h4>
                        <p class="mb-0 opacity-75" style="font-size: 14px;">
                            <i class="bi bi-shield-check me-1"></i> {{ session('role', 'Employee') }}
                            &nbsp;&bull;&nbsp;
...
<div class="row g-3 mb-4">
        ...
        <div class="card h-100 shadow-sm">
            <div class="card-header bg-white py-3">
                <h5 class="card-title mb-0">
                    {{ ($isGlobal ?? false) ? 'Presence Chart' : 'Kehadiran Saya' }}
                </h5>
            </div>
            <div class="card-body">
    ...
    <div class="col-md-6">
        <div class="card h-100 shadow-sm">
            <div class="card-header bg-white py-3">
                <h5 class="card-title mb-0">
                    {{ ($isGlobal ?? false) ? 'Payroll Chart' : 'Payroll Saya' }}
                </h5>
            </div>
            <div class="card-body">
                <div style="height: 300px; min-height: 300px; position: relative; width: 100%; overflow: hidden;">
                        ...
                        <div class="table-responsive">
                            <table class="table table-sm table-borderless">
                                ...
                                <tbody>
                                    ...
                                    <tr>
                                        <td>
                                            <span class="badge" style="background-color: {{ ['#38bdf8', '#10b981', '#fb923c', '#8b5cf6', '#a78bfa'][$index % 5] }}"> </span>
                                            {{ $label }}
                                        </td>
                                        <td class="text-end fw-bold">{{ $statusData[$index] }}</td>
                                    </tr>
                                    @endforeach
                                </tbody>
                            </table>
