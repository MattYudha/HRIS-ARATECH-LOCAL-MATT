                            ...
                            <tr>
                                ...
                                <td>
                                    <span class="badge bg-{{ 
                                        ...
                                        {{ round($data['composite_score'], 1) }}/100
                                    </span>
                                </td>
                                <td>
                                    <span class="badge bg-{{ 
                                        $data['performance_level'] === 'excellent' ? 'success' : 
                                        ($data['performance_level'] === 'good' ? 'info' : 
                                        ($data['performance_level'] === 'satisfactory' ? 'warning' : 'danger'))
                                    }}">
                                        {{ ucfirst(str_replace('_', ' ', $data['performance_level'])) }}
                                    </span>
                                </td>
                                <td>
                                    @if($index > 0)
                                        ...
                                        @endphp
                                        @if($diff > 0)
                                            <span class="text-success"><i class="bi bi-arrow-up"></i> +{{ round($diff, 1) }}</span>
                                        @elseif($diff < 0)
                                            <span class="text-danger"><i class="bi bi-arrow-down"></i> {{ round($diff, 1) }}</span>
                                        @else
                                ...
                                @foreach($categories ?? [] as $cat)
                                <td>
                                    @if(isset($data['category_scores'][$cat]))
                                        {{ round($data['category_scores'][$cat], 1) }}
                                    @else
                                        <span class="text-muted">-</span>
                                    @endif
                                </td>
            ...
            <div class="card-body">
                ...
                <div class="row">
                    <div class="col-md-4 text-center border-end">
                        ...
                        <h2 class="mb-0">{{ round($avgScore, 1) }}</h2>
                        <small class="text-muted">Over {{ $months ?? 6 }} months</small>
                    </div>
                    <div class="col-md-4 text-center border-end">
                        <h6 class="text-muted">Overall Trend</h6>
                        <h2 class="mb-0 {{ $trend >= 0 ? 'text-success' : 'text-danger' }}">
                            {{ $trend >= 0 ? '+' : '' }}{{ round($trend, 1) }}
                        </h2>
                        <small class="text-muted">{{ $trend >= 0 ? 'Improving' : 'Declining' }}</small>
                    </div>
                    <div class="col-md-4 text-center">
                        <h6 class="text-muted">Current Status</h6>
                        <h2 class="mb-0">
                            <span class="badge bg-{{ 
                                $lastScore >= 90 ? 'success' : 
                                ($lastScore >= 75 ? 'info' : 
